...
export const foundingPartnerRequests = pgTable("founding_partner_requests", {
  ...
  status: text("status").notNull().default("pending"), // pending, approved, rejected, invited, redeemed, expired
...
export const insertAudioRecordingSchema = createInsertSchema(audioRecordings).omit({ id: true, createdAt: true });
...
export const bizCompanyTypeEnum = pgEnum("biz_company_type", [
...
export const bizRelationshipStatusEnum = pgEnum("biz_relationship_status", [
  "prospect", "active", "partner", "inactive",
]);

export const bizOutreachPurposeEnum = pgEnum("biz_outreach_purpose", [
  "sales", "partnership", "founder_recruitment", "investor_outreach", "enterprise_prospect",
]);

export const bizCompanies = pgTable("biz_companies", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: text("name").notNull(),
  companyType: bizCompanyTypeEnum("company_type").notNull().default("other"),
  ...
  relationshipStatus: bizRelationshipStatusEnum("relationship_status").notNull().default("prospect"),
  outreachPurposes: text("outreach_purposes").array(),
  notes: text("notes"),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertBizCompanySchema = createInsertSchema(bizCompanies).omit({ id: true, createdAt: true, updatedAt: true });
export type BizCompany = typeof bizCompanies.$inferSelect;
export type InsertBizCompany = z.infer<typeof insertBizCompanySchema>;
