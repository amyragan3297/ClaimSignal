...
export async function registerRoutes(
  ...
  app.get("/api/biz/companies", requireAuth, requirePlatformOwner, async (req: AuthRequest, res) => {
    try {
      const companyType = req.query.companyType as string | undefined;
      const relationshipStatus = req.query.relationshipStatus as string | undefined;
  ...
  app.post("/api/biz/companies", requireAuth, requirePlatformOwner, async (req: AuthRequest, res) => {
    try {
      const { insertBizCompanySchema } = await import("@shared/schema");
      const data = insertBizCompanySchema.parse(req.body);
      const company = await storage.createBizCompany(data);
      res.status(201).json(company);
    } catch (err) { res.status(400).json({ message: (err as Error).message }); }
  });

  app.get("/api/biz/companies/:id", requireAuth, requirePlatformOwner, async (req: AuthRequest, res) => {
    try {
      const company = await storage.getBizCompany(String(req.params.id));
      if (!company) return res.status(404).json({ message: "Company not found" });
      res.json(company);
    } catch (err) { res.status(500).json({ message: (err as Error).message }); }
  });

  app.patch("/api/biz/companies/:id", requireAuth, requirePlatformOwner, async (req: AuthRequest, res) => {
    try {
      const updated = await storage.updateBizCompany(String(req.params.id), req.body);
      if (!updated) return res.status(404).json({ message: "Company not found" });
      res.json(updated);
    } catch (err) { res.status(400).json({ message: (err as Error).message }); }
  });

  app.delete("/api/biz/companies/:id", requireAuth, requirePlatformOwner, async (req: AuthRequest, res) => {
    try {
      await storage.deleteBizCompany(String(req.params.id));
      res.json({ message: "Deleted" });
    } catch (err) { res.status(500).json({ message: (err as Error).message }); }
  });
  ...
  app.get("/api/biz/outreach-list", requireAuth, requirePlatformOwner, async (req: AuthRequest, res) => {
    try {
      ...
      const list = companies
        .filter(c => c.directEmail || c.generalEmail)
        .map(c => ({
          id: c.id,
          name: c.name,
          companyType: c.companyType,
          contactPersonName: c.contactPersonName,
          email: c.directEmail || c.generalEmail,
          directPhone: c.directPhone || c.mainPhone,
          relationshipStatus: c.relationshipStatus,
          outreachPurposes: c.outreachPurposes,
          state: c.state,
        }));
      res.json(list);
    } catch (err) { res.status(500).json({ message: (err as Error).message }); }
